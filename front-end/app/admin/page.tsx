...
export default function AdminDashboard() {
  ...
  const cards = [
    { title: 'Total Students', value: statsData.totalStudents, icon: Users, color: 'bg-indigo-600' },
    { title: 'Total Instructors', value: statsData.totalInstructors, icon: ShieldCheck, color: 'bg-emerald-500' },
    { title: 'Total Courses', value: statsData.totalCourses, icon: BookOpen, color: 'bg-blue-500' },
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 md:space-y-10 animate-in fade-in duration-500">
      ...
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-2xl md:text-4xl font-black text-slate-900 tracking-tighter uppercase">Admin Dashboard</h1>
          </div>
          <p className="text-slate-500 font-medium italic text-xs md:text-sm">Real-time system-wide analytics from EduRio Database</p>
        </div>
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
           <button 
             onClick={fetchStats} 
             disabled={loading}
             className="p-3 bg-white border border-slate-100 rounded-2xl hover:shadow-xl hover:text-indigo-600 transition-all active:scale-90 group"
           >
              <RefreshCcw size={20} className={loading ? 'animate-spin' : 'group-hover:rotate-180 transition-transform duration-500'} />
           </button>
           <div className="bg-slate-900 text-white px-5 py-3 rounded-2xl flex items-center gap-3 shadow-xl shadow-slate-200 border border-slate-800">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 md:gap-6">
        {cards.map((card, i) => (
          <div key={i} className="bg-white p-6 md:p-8 rounded-[2rem] border border-slate-100 shadow-2xl shadow-slate-100/50 flex flex-col justify-between group hover:border-indigo-100 hover:-translate-y-1 transition-all duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        ...
        <div className="lg:col-span-2 bg-white p-6 md:p-10 rounded-[2.5rem] border border-slate-100 shadow-2xl shadow-slate-100/50 min-h-[400px] flex flex-col relative overflow-hidden group">
          <div className="absolute -top-24 -right-24 w-64 h-64 bg-indigo-50 rounded-full opacity-50 group-hover:scale-110 transition-transform duration-700"></div>
          ...
          <div className="relative">
            ...
            <div className="flex-1 border-2 border-dashed border-slate-200 rounded-[2rem] flex flex-col items-center justify-center bg-slate-50/50 py-20 px-6 text-center">
               <div className="p-4 bg-white rounded-full shadow-md mb-4 text-indigo-600">
                  <TrendingUp size={32} />
               </div>
               <p className="text-slate-500 font-black uppercase text-xs tracking-widest mb-1">MySQL Synchronization Active</p>
               <p className="text-slate-400 font-medium italic text-sm">Cross-role data aggregation is being monitored</p>
            </div>
          </div>
        </div>

        {/* Sidebar Status (Synced with Instructor Style) */}
             ...
             <div className="space-y-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <span className="text-lg font-black block tracking-tighter text-emerald-400">Online</span>
                  </div>
                </div>

                <div className="p-5 bg-white/5 rounded-[1.5rem] border border-white/10">
                   <p className="text-[10px] font-black text-slate-400 uppercase mb-2">Total System Admins</p>
                   <div className="flex justify-between items-end">
                      <span className="text-xl font-black italic text-rose-100">{statsData.totalAdmins} Users</span>
                      <span className="text-[10px] font-bold text-rose-400 underline cursor-pointer uppercase">Privileged</span>
                   </div>
                </div>
             </div>
          </div>
